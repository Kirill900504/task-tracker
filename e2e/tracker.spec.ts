...
test("разделы переставляются перетаскиванием", async ({ page }) => {
  ...
  // И убедиться, что перенос ДЕЙСТВИТЕЛЬНО начался, а не только что мышь
  // подвигали. Без этой строки все дальнейшие шаги — движения по
  // странице, которая ничего о них не знает, а отказ приходит в самом
  // конце и звучит как «перетаскивание не работает».
  await expect(src).toHaveClass(/dragging/, { timeout: 5000 });

  // Ведём к началу ряда одним махом, без пауз между шагами, — и это часть
  // проверки, а не спешка.
  //
  // Перестановка когда-то определяла цель по элементу под курсором, то
  // есть по DOM, который перестраивает React. Мышь (и тем более
  // Playwright) успевает прислать несколько движений раньше первой
  // перерисовки, и каждое следующее считалось по ряду, которого на экране
  // уже нет: раздел проезжал одну позицию и замирал. Теперь позиция
  // считается по снимку ряда, снятому в начале переноса
  // (lib/sectionOrder), — значит быстрая рука обязана доводить до конца.
  //
  // Цель — левее первого раздела, а не середина соседа: раздел встаёт
  // туда, где курсор, и остановка в середине соседа означала бы «после
  // него».
  const first = (await page.locator("#sectionTabs .section-tab[data-section-id]").first().boundingBox())!;
  await page.mouse.move(first.x + 6, first.y + first.height / 2, { steps: 8 });
  await page.mouse.up();
