...
test("принятая задача открывается заново кнопкой в карточке", async ({ page }) => {
  ...
  await approve.click();
  // Комментарий к приёмке обязателен с 23.09.2026 («приёмка без
  // комментария не решение») — пустое нажатие «ОК» оставляет окно
  // открытым и пишет под полем требование заполнить его, а не закрывает
  // его молча. Раньше здесь хватало пустого подтверждения.
  await page.fill("#askInput", "Готово, принято");
