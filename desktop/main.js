...
function createWindow() {
  ...
  // Сначала заставка, лежащая рядом на диске, и только потом сайт.
  //
  // Слова Кирилла 20.09.2026: «при запуске приложения сперва появляется
  // чёрный экран и в левом верхнем углу слово „Загрузка…“». Слово с
  // экрана убрано на стороне сайта (там теперь каркас трекера), но
  // остаётся то, что было до него: окно ждёт `ready-to-show`, а первый
  // кадр сайта не может появиться раньше, чем ответит сеть. В эти
  // полсекунды-секунду человек видит либо пустой фон, либо вообще
  // ничего — приложение как будто не открылось.
  //
  // Заставка снимает именно это: она локальная, рисуется мгновенно, и
  // окно появляется сразу со знаком и строкой «Открываю трекер…». Сайт
  // грузится следом, поверх неё. Держать её дольше нечем и незачем —
  // она исчезает в тот момент, когда трекер готов показать себя.
  void mainWindow.loadFile(path.join(__dirname, "splash.html")).then(() => {
...
function checkForUpdates({ tellMe = false } = {}) {
  ...
  // Скачали — спрашиваем один раз, ставить ли сейчас.
  //
  // Тихая установка «при следующем выходе» выглядит разумно и на практике
  // означает, что новая версия приезжает через раз: один запуск скачивает,
  // и только СЛЕДУЮЩИЙ закрывает и ставит. Кирилл 20.09.2026 открыл
  // приложение и увидел старое название со старой шапкой — ровно этот
  // случай, и объяснить его человеку нечем: он всё сделал правильно.
  // Поэтому здесь один короткий вопрос, и только когда файл уже на диске:
  // обновления оболочки выходят несколько раз в год, а «Позже» оставляет
  // прежнее поведение — поставится при выходе.
  if (!tellMe) {
    autoUpdater.once("update-downloaded", (info) => {
      void dialog
        .showMessageBox({
          type: "question",
          message: `Готово обновление ${info?.version ? "до версии " + info.version : "приложения"}.`,
          detail: "Перезапустить сейчас? Это займёт несколько секунд. Задачи и встречи никуда не денутся — они в облаке.",
