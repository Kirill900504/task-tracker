...
export async function POST(req: Request) {
  ...
  // Владелец не может принять приглашение в собственный трекер.
  //
  // Ссылка открывается в том же браузере, где он уже вошёл, — и вход у него
  // обычно есть. Без этой проверки его собственная учётная запись
  // становится участником своего же пространства, а трекер после этого
  // показывает ему экран руководителя вместо его трекера: NewTracker
  // выбирает экран по строке в workspace_members, и она теперь есть.
  // Разбираться с этим пришлось бы через базу.
  if (sessionUser?.id === invite.owner_id) {
    return NextResponse.json(
      {
        error:
          "Это ваш собственный трекер — принять в него приглашение нельзя. " +
          "Откройте ссылку в приватном окне или выйдите из аккаунта: вход руководителя должен быть отдельным.",
      },
      { status: 400 },
    );
  }
