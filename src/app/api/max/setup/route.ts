...
//
// Единственное, что может сделать только человек, — получить токен у
// MasterBot внутри самого MAX. Всё остальное (проверить токен, узнать имя
// бота, придумать секрет, подписать вебхук, сохранить) делается здесь,
...
export async function POST(req: Request) {
  ...
  if (!info.username) {
    return NextResponse.json(
      { error: "Токен рабочий, но у бота нет имени (@username). Задайте его в MasterBot и вставьте токен ещё раз." },
      { status: 400 },
    );
  }
