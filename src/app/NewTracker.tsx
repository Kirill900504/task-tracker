...
export default function NewTracker() {
  ...
  // наблюдатели, а у встречи — приглашённые. `useTaskParticipants()` жил
  // внутри `TasksPanel` — поднят сюда целиком, ОДНИМ экземпляром на весь
  // трекер, а не заведён здесь ВТОРЫМ: у него своя подписка на
  // task_participants, и две независимые копии перезагружаются не
  // синхронно. Ровно это 23.09.2026 уронило e2e «принятая задача
  // открывается заново кнопкой в карточке» — окно закрывалось по одному
  // каналу, а видимость (значит и то, что рисует TasksPanel) ещё
  // мгновение ехала по другому. Экземпляр один, `participants` идёт вниз
  // пропом — тот же приём, что уже применён для tasks/meetings/ideas.
