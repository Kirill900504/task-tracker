...
export type BotTransport = {
  ...
  resolveCallback(input: {
    ...
    // Следом за нажатием идёт ещё одно сообщение — список, карточка,
    // подтверждение. В Telegram это ничего не меняет: toast там всплывашка
    // и стоит бесплатно. В MAX всплывашек нет, и toast приходится слать
    // СООБЩЕНИЕМ — то есть человек получал «Открываю», а следом сам ответ,
    // и так на каждое нажатие (ровно это видно на снимке чата 22.09.2026).
    // Слово, которое ничего не добавляет к тому, что придёт через
    // полсекунды, — лишняя строка в истории и лишний вызов в Россию.
    more?: boolean;
  }): Promise<void>;
};
