...
describe("sameLayout", () => {
  it("treats a layout with the same content but different key order as equal", () => {
    ...
    const fromDatabase = {
      ...
      right: ["weekPanel", "peoplePanel", "ideasPanel"],
      center: ["mainCol"],
    } as PanelLayout;
  ...
  it("spots a panel moved to another zone", () => {
    const moved: PanelLayout = { left: ["calPanel"], center: ["mainCol", "meetingsPanel"], right: ["weekPanel", "peoplePanel", "ideasPanel"] };
    expect(sameLayout(moved, DEFAULT_PANEL_LAYOUT)).toBe(false);
  });

  it("spots panels reordered within a zone", () => {
    const reordered: PanelLayout = { left: ["meetingsPanel", "calPanel"], center: ["mainCol"], right: ["weekPanel", "peoplePanel", "ideasPanel"] };
