...
describe("buttons", () => {
  ...
  it("наблюдателю не предлагает отчитаться", () => {
    // Его поставили знать, а не отвечать: «Сделал» от наблюдателя — это
    // отчёт, которого никто не ждал, и постановщик решит, что работу сделал
    // человек, которого на неё не ставили.
    expect(taskButtons("t42", "watcher")).toEqual([]);
    expect(taskButtons("t42", "coexecutor")[0]).toHaveLength(2);
    expect(taskButtons("t42")[0]).toHaveLength(2);
  });
