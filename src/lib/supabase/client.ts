...
export function createClient() {
  // Прогреть решение о маршруте СРАЗУ, а не в момент первого запроса.
  //
  // Без этого проба (до PROBE_TIMEOUT_MS на сети, которая молча роняет
  // прямой хост, — ровно та, ради которой этот файл существует) начиналась
  // одновременно с первым же чтением данных в useTrackerData и добавлялась
  // ПОВЕРХ его короткого бюджета на быстрый старт (FAST_PROBE_MS), почти
  // гарантированно съедая его целиком и поднимая баннер «нет связи» на
  // ровном месте — 24.09.2026, «замучали эти ошибки». Здесь она стартует
  // раньше и идёт параллельно с чтением сессии, так что к моменту первого
  // запроса к таблицам маршрут обычно уже решён. route() сам по себе не
  // делает лишней работы при повторном вызове — решение кешируется.
  void route();
