...
describe("applyLocalChanges", () => {
  ...
  it("считает строку удалённой, если она есть в shadow и нет у сервера — и поэтому снимок берут ДО чтения", () => {
    ...
    //
    // Чинится это не здесь, а моментом съёмки: в catchUp половины пары
    // берутся в РАЗНЫЕ секунды — shadow до запроса (тогда такая строка
    // выглядит работой, которая ещё не уехала), а live на момент
    // применения ответа (иначе теряется уже обратное: галочка, нажатая
    // за ту же секунду чтения, стиралась бы серверной версией). Тест
    // стоит затем, чтобы правило нельзя было «починить» здесь — соседний
    // тест про удаление с другого устройства сломался бы первым.
