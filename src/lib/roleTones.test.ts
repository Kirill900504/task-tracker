...
describe("корешок роли на карточке", () => {
  ...
  it("у исполнителя фон, а не только полоска", () => {
    // 22.09.2026: «исполнителю верни выделение в фирменном цвете как
    // раньше». Две полоски одного семейства различаются только
    // насыщенностью, то есть их приходится сравнивать; свою работу ищут
    // глазами через всю доску, и ей нужен фон. Исключение ровно одно —
    // заливка у «помогаю» и «смотрю» вернула бы три близких фона.
    const rule = css.match(/\.task\.role-executor\{[^}]*\}/)?.[0] || "";
    expect(rule, "правило .task.role-executor пропало").toMatch(/background/);
    expect(css).not.toMatch(/\.task\.role-coexecutor\{[^}]*background/);
    expect(css).not.toMatch(/\.task\.role-watcher\{[^}]*background/);
  });
