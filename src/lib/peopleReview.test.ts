...
describe("что считается по человеку", () => {
  ...
  it("считает, сколько человек ДЕЛАЕТ, а не только сколько думает", () => {
    // Две цифры отвечают на разные вопросы. Две просрочки у того, кто
    // берётся мгновенно и делает три недели, и у того, кто неделю не
    // открывает задачу, — это разные разговоры.
    const [s] = personStats(
      [
        row({ acceptedAt: "2026-09-10T09:00:00Z", doneAt: "2026-09-10T15:00:00Z" }), // 6 ч
        row({ acceptedAt: "2026-09-10T09:00:00Z", doneAt: "2026-09-10T19:00:00Z" }), // 10 ч
        row({ doneAt: "2026-09-10T19:00:00Z" }), // отчитался, не приняв — не в счёт
      ],
      NOW,
    );
    expect(s.avgWorkHours).toBe(8);
  });

  it("отчёт без «принял» не засчитывается как мгновенная работа", () => {
    // Иначе молчание выглядело бы образцовой скоростью.
    const [s] = personStats([row({ doneAt: "2026-09-10T19:00:00Z" })], NOW);
    expect(s.avgWorkHours).toBeNull();
  });
