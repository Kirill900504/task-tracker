...
export async function handleBotCallback(
  ...
  if (owner) {
    ...
    if (!outcome) {
      // Кнопка не владельческая — но это ещё не значит «чужая». Владелец
      // сам бывает исполнителем: задача, поставленная ему руководителем,
      // приходит в тот же чат с кнопками «Принял» и «Сделал», и владелец
      // в этот момент — обычный участник. Поэтому нажатие сначала
      // пробуется как исполнительское и только потом объявляется чужим:
      // иначе половина кнопок в его чате молча не работала бы.
