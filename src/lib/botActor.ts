...
// читается лучше.
//
// Проверка на пустоту здесь не паранойя, а шрам. 20.09.2026 в одну из
// веток вместо актора приехала голая строка с id: Supabase без
// сгенерированных типов отдаёт поля как `any`, и тип этого не поймал.
// Дальше случилось худшее из возможного — не пустой список, а обратное:
// `actor.spaceId` у строки не существует, в обе колонки лёг `undefined`,
// а `.match()` ключи с `undefined` просто ВЫБРАСЫВАЕТ. Запрос ушёл вовсе
// без фильтра по пространству, и на вопрос «что сегодня» бот показал
// девятнадцать задач вместо восьми — чужие пространства целиком.
//
// Поэтому фильтр, который не может отфильтровать, не строится вовсе:
// пусть упадёт вызов и об этом расскажет журнал, чем тихо уедет выборка
// по всей базе. Единственный безопасный ответ на «я не знаю, чьё это» —
// не спрашивать базу.
export function actorScope(actor: BotActor): Record<string, string> {
  if (!actor?.spaceId || (!actor.isOwner && !actor.userId)) {
    throw new Error("actorScope без актора: выборка ушла бы без границы пространства");
  }
