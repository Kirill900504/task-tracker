...
export default function TaskAnswer({
  ...
  // уровни задачи или встречи». И оттуда же его претензия про волевое
  // закрытие: «задача из кан-бана пропадает, а открытая её форма остаётся
  // перед глазами».
  //
  // Правило поэтому не «после отчёта», а ШИРЕ: закрываем окно тогда,
  // когда ответ ПЕРЕДВИНУЛ задачу. «Сделал» и «Не могу» — оба такие: с
  // тех пор как отказ считается ответом (см. taskProgress.allAnswered),
  // он уводит задачу на приёмку точно так же, как отчёт, и висящее над
  // ней окно в прежнем столбце — ровно тот симптом, с которым он и
  // пришёл.
  //
  // «Прошу перенос» окно НЕ закрывает, и это не забывчивость: просьба
  // никуда задачу не двигает — она остаётся там же и ждёт решения
  // постановщика, а человек продолжает читать её карточку.
  //
  // Зовётся только после УСПЕШНОГО ответа: не ушло — окно остаётся, и
  // причина видна в нём же.
  onAnswered?: () => void;
}) {
