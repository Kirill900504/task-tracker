...
export default function MeetingsPanel({
  ...
  meId?: string;
  // Что показывает список/доску встреч: уже сужено до «своего» — где я
  // организатор или приглашённый (см. NewTracker.tsx, 23.09.2026, «каждый
  // видит только своё, включая владельца»).
  meetings: Meeting[];
  // Занятость людей на выбранный день (busyByTime в MeetingModal) — это
  // единственное место в трекере, где нарочно нужны ВСЕ встречи
  // пространства, а не только свои: приглашая человека, надо видеть, что
  // он занят, даже если сама его встреча тебе не видна. Не название, не
  // повестка — только дата/время/участники, и то лишь для этой ровно
  // одной проверки. Без этого прежнего поведения «Есина, Мамакова и
  // Макаров заняты в 12:00» просто перестало бы работать после сужения
  // видимости. Опционально — если не передали, используется `meetings`
  // (как было раньше, для мест, где панель и так держит всё).
  allMeetings?: Meeting[];
  assignees: string[];
    ...
    <div className="panel dash-panel" id="meetingsPanel" data-panel-id="meetingsPanel" ref={setMeetingsDropRef}>
      ...
      {modalOpen && (
        <MeetingModal
          ...
          // Чем заняты люди в этот день — считается из ВСЕХ встреч
          // пространства (allMeetings), а не только видимых мне: занятость
          // человека, приглашённого в новую встречу, обязана учитывать и ту
