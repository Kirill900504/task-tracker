...
//
// Было 2500 — и это оказалось короче, чем сама проверка маршрута
// (supabase/client.ts: до 4000мс на сети, которая молча роняет прямой
// Supabase-хост и требует нашего /sb). Та проверка идёт ПЕРЕД любым первым
// запросом, и на такой сети (мобильный интернет, обычное дело для этого
// трекера) быстрая проба почти гарантированно не успевала — баннер «нет
// связи» вспыхивал на каждом открытии, даже когда секундой позже все данные
// приходили нормально. 24.09.2026, «замучали эти ошибки». Теперь запас
// шире худшего случая той проверки: у настоящего оффлайна (самолёт, вообще
// без сети) проба падает быстро сама, за счёт мгновенного отказа fetch, а
// не по этому таймауту — так что здесь можно позволить себе разницу.
const FAST_PROBE_MS = 5500;
