...
export interface Task {
  ...
  createdBy?: string;
  // Когда задачу поставили. Только для чтения, как и createdBy: колонка
  // заполняется базой (`default now()`), а таблица taskToRow её не пишет.
  // Нужна карточке: «поставлена 6 ноября» — половина ответа на вопрос
  // «почему она до сих пор здесь», и Кирилл попросил её прямо
  // (21.09.2026: «кто постановщик → напротив дата постановки задачи»).
  createdAt?: string;
}
...
export interface Meeting {
  ...
  createdBy?: string;
  // Когда встречу назначили — та же пара к createdBy, что у задачи, и
  // показывается она в карточке на том же месте.
  createdAt?: string;
}
