...
`C:\Users\Kirill\.claude\unfinished\<дата>-<короткое-имя>.md`: каталог
проекта и ветка; что сделано и закоммичено, а что лежит незакоммиченным;
что осталось — по шагам, без «и так далее»; чего делать НЕ надо (чужие
файлы соседних сессий, решения, которые он уже принял); чем проверять.
Пишется она для человека, который этого разговора не видел, — следующая
сессия его действительно не увидит. Каталог лежит ВНЕ репозитория
нарочно: записки не код и в историю проекта им незачем.

*Дежурный.* Запланированная задача «Доделать незаконченное»
(`~/.claude/scheduled-tasks/finish-unfinished/`) раз в час открывает новую
сессию, берёт САМУЮ СТАРУЮ записку — одну за прогон, — доводит её план до
конца и переносит файл в `unfinished\done\`. Не восстановились лимиты —
прогон просто не отработает и повторится через час. Работает это, пока
открыто приложение.
...
проспал целиком*, и все три причины стоит знать наперёд.
**Записки не было.** Она пишется сессией, которая ПОНЯЛА, что не
успевает; сессию, которую обрывают на лимите, не предупреждают, и хода
на файл у неё уже нет. Каталог `unfinished\` был пуст, а в рабочей копии
сутки лежала готовая работа (корешок роли на карточке) — незакоммиченная
и потому невидимая. След обрыва остаётся всегда и он один: `git status`.
...
писать живая сессия.
**Часы дежурного идут, только пока открыто приложение.** Сутки закрытого
ноутбука — сутки без единого прогона, и это не чинится кодом: сказать об
этом вслух — единственное, что здесь можно сделать.
**И зависший прогон блокирует все следующие.** Прогон 21.09 встал на
первом же вызове (приложение закрыли посреди него) и остался числиться
running; запуск следующего отвечает «already running», то есть дежурный
мёртв, пока эту сессию не остановят руками. Остановить её из соседней
сессии нельзя — классификатор безопасности не даёт, — так что это кнопка
«Стоп» у Кирилла. **Если дежурный молчит больше суток — смотреть надо не
записки, а список его прогонов.**
