...
следующего догона. Причина — гонка между `catchUp()` в
`useTrackerData.ts` и локальной правкой, случившейся ПОКА идёт его
`loadAll()`: снимок «что показано / что подтверждено» берётся ДО запроса
(см. комментарий там же — это чинит другую потерю), но решение «доверять
серверу или слить» и сравнение «что реально изменилось» тоже читают этот
снимок, а не текущее состояние на момент, когда ответ пришёл. `waitForSaved`
между действиями снижает вероятность, но не убирает её — таймер догона
независим от теста. Пока это не починено (в работе, судя по вечеру
23.09.2026), тест выше слегка нестабилен НЕ из-за сортировки, которую он
проверяет, — она проверена руками и работает всегда; повторный прогон
