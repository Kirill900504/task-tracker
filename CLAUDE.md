...
конца и удаляет файл. Не восстановились лимиты — прогон просто не
отработает и повторится через час. Работает это, пока открыто приложение.

Отсюда два следствия, и они важнее механики. **Записка удаляется только
вместе с готовой работой**: пока файл лежит, к нему возвращаются каждый
час. И **дежурный не выдумывает** — если в записке чего-то не хватает или
решение принимать Кириллу, он дописывает в неё «Что мешает» и
останавливается, а не решает за него.
