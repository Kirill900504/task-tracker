...
фирменный тон, соисполнителю тот же фирменный вполсилы, наблюдателю
светло-серый (`lib/myRole.ts`, переменные `--role-*` в `tracker.css`).
Важность стала точкой в углу именно поэтому: «высокий
...
`src/lib/roleTones.test.ts` считает расстояние между `--role-*` прямо по
файлу стилей, а `e2e/roles.spec.ts` заводит четыре задачи с четырьмя
ролями и спрашивает у браузера вычисленный фон — первый ловит цифру,
второй то, что правило вообще доехало до страницы.
