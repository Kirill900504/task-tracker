...
*npm 11 не выполняет install-скрипты без разрешения*, и разрешение в
 пиннуто к точной версии. Обновление lock-файла молча
оставляет Electron без бинарника — «npm ci» отрабатывает за двенадцать
секунд и падает потом, внутри electron-builder. Поэтому в workflow стоит
явный , безвредный, когда бинарник
уже на месте.
...
аннотацией () — аннотации API отдаёт публично. Тот же приём
уже был у schema-теста в ci.yml; если добавляете шаг, который может
упасть, добавьте и это.
