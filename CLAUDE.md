...
человек видит знакомый адрес трекера. Письмо «Забыли пароль?» на странице
входа этим НЕ лечится: его текст формирует сам Supabase, и пока Site URL
проекта — localhost, оно ведёт в никуда (Authentication → URL Configuration).
...
- **Коммитьте СВОИ файлы, а не `git add -A`.** Общее дерево означает, что
  «всё» — это и чужое недоделанное. Перечисляйте пути руками.
