...
отчитаться, в «Завершённые» — принять работу, обратно — вернуть. У
каждого перехода есть тот, кому он позволен (`moveBetween`), и отказ
произносится словами, а не молчаливым возвратом карточки. Отчёт, приёмка
и возврат требуют комментария, поэтому перенос ОТКРЫВАЕТ карточку, а не
пишет за человека.
