...
**И четвёртый, живой, а не средовой: догон (`lib/revive.ts`, появился
22.09.2026) может перечитать список между двумя быстрыми действиями подряд
и потерять то, что между ними случилось.** Пойман 23.09.2026 на
«вычеркнутые мысли идут свежими сверху»: две мысли заводятся и
вычёркиваются подряд, и вторая иногда либо возвращается активной, либо
пропадает из панели вовсе — притом что в базе всё цело, стоит подождать
следующего догона. Причина была в `catchUp()` (`useTrackerData.ts`):
снимок «что показано / что подтверждено» брался ДО запроса целиком, одним
куском, а решение «доверять серверу или слить» и сравнение «что реально
изменилось» читали этот же снимок вместо текущего состояния на момент,
когда ответ пришёл. Починено в 82d1a2a: половины пары теперь снимаются в
разные моменты — shadow до запроса (иначе строка, подтвердившаяся во время
чтения, читалась бы как чужое удаление), live после ответа (иначе правка,
случившаяся за то же время, терялась бы). Падений стало заметно меньше —
2 из 15 против прежних 2 из 5, — но не ноль: 23.09.2026 после фикса тот же
тест один раз поймал соседний край, где вторая мысль пропадала из панели
целиком, ДАЖЕ НЕ будучи тронутой в эту итерацию (порядок в тесте такой:
первая уже вычеркнута и подтверждена, вторая ещё активна — и вот её-то и не
находит локатор). Если тест выше падает — это, скорее всего, снова он, а не
регрессия сортировки, которую он проверяет (она проверена руками и работает
всегда); повторный прогон отличает одно от другого.
